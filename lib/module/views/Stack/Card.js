...
import { View, I18nManager, StyleSheet, Platform, InteractionManager } from 'react-native';
...
const {
  ...
  Clock,
  Value
} = Animated; // We need to be prepared for both version of reanimated. With and w/out proc

let memoizedSpring = spring; // @ts-ignore

if (Animated.proc) {
  // @ts-ignore
  const springHelper = Animated.proc((finished, velocity, position, time, prevPosition, toValue, damping, mass, stiffness, overshootClamping, restSpeedThreshold, restDisplacementThreshold, clock) => spring(clock, {
    finished,
    velocity,
    position,
    time,
...
function transformSpringConfigToAnimatedValues(config) {
  return {
    ...
    restDisplacementThreshold: new Animated.Value(config.restDisplacementThreshold),
...
export default class Card extends React.Component {
  constructor(...args) {
    ...
    })), onChange(this.gestureUntraversed, set(this.didMovementHappen, 1)), cond(eq(this.gestureState, GestureState.ACTIVE), [cond(this.isSwiping, NOOP_NODE, [// We weren't dragging before, set it to true
    ...
    set(this.props.current, min(max(sub(this.offset, cond(this.distance, divide(cond(eq(this.direction, DIRECTION_HORIZONTAL), multiply(this.gestureUntraversed, I18nManager.isRTL ? MINUS_ONE_NODE : TRUE_NODE), this.gestureUntraversed), this.distance), TRUE_NODE)), FALSE_NODE), TRUE_NODE)), // Stop animations while we're dragging
    ...
    })), stopClock(this.clock)], [set(this.isSwipeCancelled, eq(this.gestureState, GestureState.CANCELLED)), set(this.isSwiping, FALSE_NODE), this.runTransition(cond(greaterThan(abs(this.extrapolatedPosition), divide(this.distance, 2)), cond(lessThan(cond(eq(this.velocity, FALSE_NODE), this.gesture, this.velocity), FALSE_NODE), TRUE_NODE, FALSE_NODE), this.isVisible))])]));

    _defineProperty(this, "handleGestureEventHorizontal", Animated.event([{
      nativeEvent: {
    ...
    _defineProperty(this, "handleGestureEventVertical", Animated.event([{
      nativeEvent: {
  ...
  render() {
    ...
    if (cardStyle) {
      const style = StyleSheet.flatten(cardStyle);
    ...
    return /*#__PURE__*/React.createElement(StackGestureContext.Provider, {
      value: this.gestureRef
    }, /*#__PURE__*/React.createElement(View, _extends({
      pointerEvents: "box-none"
    }, rest), /*#__PURE__*/React.createElement(Animated.Code, {
      key: gestureEnabled ? 'gesture-code' : 'no-gesture-code',
      exec: gestureEnabled ? this.execWithGesture : this.execNoGesture
    }), overlayEnabled && overlayStyle ? /*#__PURE__*/React.createElement(Animated.View, {
      pointerEvents: "none",
      style: [styles.overlay, overlayStyle]
    }) : null, overlayBackgroundComponent && overlayBackgroundComponent(), /*#__PURE__*/React.createElement(Animated.View, {
    ...
    }, shadowEnabled && shadowStyle && !transparent ? /*#__PURE__*/React.createElement(Animated.View, {
      style: [styles.shadow, gestureDirection === 'horizontal' ? styles.shadowHorizontal : styles.shadowVertical, shadowStyle],
      pointerEvents: "none"
    }) : null, /*#__PURE__*/React.createElement(PointerEventsView, {
      active: active,
